...
import { TemplateBottom, TemplateTopLeft, TemplateCentered, TemplateMinimal, TemplatePolaroid, TemplateMagazine, TemplateCinematic, TemplateGradient, TemplateDuotone, TemplateNewspaper, TemplateLetterboxd, TemplateType, FontType, ColorTheme, TextStyle, TextAlign, QuoteStyle } from '@/components/StoryTemplates';
...
export default function Home() {
  ...
  const loadReviewFromUrl = async (reviewUrl: string) => {
    ...
    try {
      ...
      // Fetch available backdrops from TMDB
      if (data.movieTitle) {
        try {
          const params = new URLSearchParams({ title: data.movieTitle, ...(data.year ? { year: data.year } : {}) });
          const bdRes = await fetch(`/api/movie-backdrops?${params}`);
          const bdData = await bdRes.json();
          if (bdData.backdrops?.length) {
            setAvailableBackdrops(bdData.backdrops);
          }
        } catch (e) {
          console.warn('[Backdrops] Failed to fetch:', e);
        }
      }
    } catch (err) {
  ...
  const handleRandomize = useCallback(() => {
    // 1. Template
    const templates: TemplateType[] = ['bottom', 'topLeft', 'centered', 'minimal', 'split', 'magazine', 'cinematic', 'gradient', 'duotone', 'newspaper', 'letterboxd'];
